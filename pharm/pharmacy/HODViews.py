...
def managePharmacist(request):
    doctors = Pharmacist.objects.all()
    context = {
        "doctors": doctors,
...
def managePharmacyClerk(request):
    ...
    doctors = PharmacyClerk.objects.all()
    context = {
        "doctors": doctors,
...
def hodProfile(request):
    ...
    doctor=AdminHOD.objects.get(admin=customuser.id)
    ...
    if request.method == 'POST':
        ...
        doctor=AdminHOD.objects.get(admin=customuser.id)
        form =HodForm(request.POST,request.FILES,instance=doctor)
...
def deletePharmacyClerk(request,pk):
    ...
    return render(request,'hod_templates/sure_delete.html')
def editPharmacist(request,doctor_id):
    doctor = Pharmacist.objects.get(admin=doctor_id)
    if request.method == "POST":
